...
then
    ...
    bash /etc/wireguard/addwguser.sh -s /etc/wireguard/server.conf -u $1 -f $2 -r 0.0.0.0/0 -ns 10.100.100.254
...
then
    ...
    bash /etc/wireguard/addwguser.sh -s /etc/wireguard/testserver.conf -u $1 -f $2 -r 10.31.31.0/24,10.100.0.0/16 -d client2 -a 10.100.200.$(echo $userip | sed 's/.*\.//g' | sed 's/\/.*//g') -ns 10.100.200.254
